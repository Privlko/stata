...
qui { // Module 8 - write a log file
	...
	!echo #########################################################################  >> "`out'.log"
	!echo #                                                                          >> "`out'.log"
	!echo # The original data set has been processed for down-stream use; a number   >> "`out'.log" 
	!echo # of standard quality control routines have beed applied;                  >> "`out'.log"
	!echo # 1- any duplicate observation were removed                                >> "`out'.log"  
	!echo # 2- if duplicate identifiers were observed with different association     >> "`out'.log"
	!echo #    results, both were removed                                            >> "`out'.log"
	!echo # 3- all data was aligned to hg19+1 using 1000-genomes-reference data. we  >> "`out'.log"
	!echo #    also convert allele code to the same strand of the reference data -   >> "`out'.log"
	!echo #    this has many advantages and disadvantages such as;                   >> "`out'.log"
	!echo #    a- data-loss as not all markers in the original gwas is included on   >> "`out'.log"
	!echo #       the 1000-genomes reference (rsid) may have been updated to a newer >> "`out'.log"
	!echo #       build                                                              >> "`out'.log"
	!echo #    b- data-loss as some markers have ambiguous genotypes - making strand >> "`out'.log"
	!echo #       alignment difficult to impossible; these markers are also dropped. >> "`out'.log"
	!echo #       we also drop indels too.                                           >> "`out'.log"
	!echo #    c- data-cleaning as some markers reveal genotypes that are            >> "`out'.log"
	!echo #       incompatible for the reference or alternative strand               >> "`out'.log"
	!echo # 4- we remove imputed markers if we have an info of <.8                   >> "`out'.log"
	!echo # 5- we remove meta-analysed markers if they were absent from more than 1  >> "`out'.log"
	!echo #    of the contributing datasets                                          >> "`out'.log"
	!echo # 6- finally we add a dummy allele frequency if an allele frequency is     >> "`out'.log"
