...
di as text"#########################################################################"
di as text"recodestrand - version 0.1a - 05May2017 richard anney "
di as text"#########################################################################"
di as text"# allele 1 for the reference strand is     `ref_a1'"
di as text"# allele 2 for the reference strand is     `ref_a2'"
di as text"# allele 1 for the data to be converted is `alt_a1'"
di as text"# allele 2 for the data to be converted is `alt_a2'"
di as text"#########################################################################"
di as text"# - this script flags all indel, ambiguous, missing, monomorphic markers"
di as text"#   in the variable _tmpflag (1 = error)"
di as text"# - this script creates new allele codes for the data (_tmpb1 and _tmpb2)"
di as text"# - this script creates flip code for all markers that were flipped (_tmpflip)"
di as text"#########################################################################"
di as text"# Started: $S_DATE $S_TIME"
di as text"#########################################################################"
di as text"# > generating genotype variable for reference alleles"
qui {
	qui recodegenotype, a1(`ref_a1') a2(`ref_a2')
	rename _gt temp_ref_gt
	}
di as text"# > generating genotype variable for test alleles"
qui {	
	qui recodegenotype, a1(`alt_a1') a2(`alt_a2')
	rename _gt temp_alt_gt
	}
di as text"# > checking for incompatible genotypes"
qui {
	noi di as text"# >> tabulating genotypes"
	noi ta temp_ref_gt temp_alt_gt
	noi di as text"# >> dropping non- R Y M K genotypes"
	foreach xx in A C G T ID DI W S {
		drop if temp_ref_gt == "`xx'"
		drop if temp_alt_gt == "`xx'"
		}
	noi di as text"# >> tabulating R Y M K genotypes (clean #1)"
	noi ta temp_ref_gt temp_alt_gt
	noi di as text"# >> dropping non-compatible genotypes (K!=M and R!=Y)"
	...
	noi di as text"# >> tabulating genotypes (clean #2)"
	noi ta temp_ref_gt temp_alt_gt
	}
di as text"# > checking for strand flips"
qui {
	gen _tmpflip = 0
	replace _tmpflip = 1 if temp_ref_gt != temp_alt_gt
	}
di as text"# > creating new variable for genotypes needing to be flipped"
qui {
	gen _tmpb1 = `alt_a1'
	gen _tmpb2 = `alt_a2'
...
di as text"# > creating alternative genotype from new alleles"
qui {
	qui recodegenotype, a1(_tmpb1) a2(_tmpb2)
	}
di as text"# > tabulating genotypes (clean #3)"
qui {
